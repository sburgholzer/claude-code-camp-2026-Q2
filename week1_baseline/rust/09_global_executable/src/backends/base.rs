...
pub trait Backend {
    ...
    /// Snake_case-able provider name, e.g. `"Anthropic"`, `"OllamaCloud"`.
    /// A dyn-safe instance-method wrapper around `backend_name()` (which
    /// itself requires `Self: Sized` and so can't be called through a
    /// trait object) — each impl just delegates to it.
    fn name(&self) -> &'static str;

    /// The validated model string this backend instance was constructed
    /// with.
    fn model(&self) -> &str;
...
pub trait PromptBackend<T: Task>: Backend {
    fn to_payload(
        &self,
        context: &Context<T>,
