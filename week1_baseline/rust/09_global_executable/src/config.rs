    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ConfigError::MissingSetting { task, key } => {
                write!(f, "tasks.{task}.{key} is required in settings.yaml")
            }
        }
    }
}
...
impl Config {
    ...
    fn resolve_dir() -> PathBuf {
        // 1. Explicit override
        if let Ok(raw) = env::var("BOUKENSHA_DIR") {
            let expanded = Self::expand_tilde(&raw);
            return std::path::absolute(&expanded).unwrap_or(expanded);
        }

        // 2. ~/.boukensharc's BOUKENSHA_DIR=... line
        if let Some(raw) = crate::rc::read().get("BOUKENSHA_DIR") {
            let expanded = Self::expand_tilde(raw);
            return std::path::absolute(&expanded).unwrap_or(expanded);
        }

        // 3. ~/.boukensha default
        let expanded = Self::default_dir();
