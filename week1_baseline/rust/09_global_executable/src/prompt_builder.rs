...
use crate::backends::{Backend, ParsedResponse, PromptBackend};
...
impl<T: Task> PromptBuilder<T> {
    ...
    /// Type-erased backend metadata accessor (name/model/usage_unit/
    /// usage_level/estimate_cost), for `Logger`, which stays non-generic
    /// over `T: Task`.
