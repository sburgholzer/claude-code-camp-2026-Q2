...
pub struct Client<'a, T: Task> {
    builder: &'a PromptBuilder<'a, T>,
    agent: Agent,
}

impl<'a, T: Task> Client<'a, T> {
    pub fn new(builder: &'a PromptBuilder<'a, T>) -> Self {
    ...
    pub fn call(&self, max_output_tokens: u32) -> Result<serde_json::Value, ApiError> {
        let url = self.builder.url();
        let headers = self.builder.headers();
        let body = serde_json::to_string(&self.builder.to_api_payload(max_output_tokens))
