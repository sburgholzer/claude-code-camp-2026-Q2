...
pub trait Task {
    ...
    fn max_iterations(settings: &Settings) -> u32 {
        Self::fetch(settings, "max_iterations")
            .and_then(Value::as_u64)
            .map(|v| v as u32)
            .unwrap_or(Self::DEFAULT_MAX_ITERATIONS)
    }

    fn max_output_tokens(settings: &Settings) -> u32 {
        Self::fetch(settings, "max_output_tokens")
            .and_then(Value::as_u64)
            .map(|v| v as u32)
