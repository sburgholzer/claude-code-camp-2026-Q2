...
pub struct Logger {
    ...
    pub path: PathBuf,
    log_io: File,
    subscribers: Vec<Box<dyn FnMut(&serde_json::Value)>>,
}
...
impl Logger {
    pub fn new(
        ...
        let mut logger = Self { session_id, path, log_io, subscribers: Vec::new() };
    ...
    pub fn turn(&mut self, n: u32) {
        self.write_log(json!({"phase": "turn", "n": n}));
    }
    ...
    pub fn subscribe(&mut self, callback: impl FnMut(&serde_json::Value) + 'static) {
